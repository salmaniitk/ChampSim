...
uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    //////////////////////////////////Attack Prevention /////////////////////////////////by salman
    
    
    // Only happens at LLC layer
    	if(cache_type == IS_LLC){
    		//cout << "cpu(invalid): ";
    		for (way=0; way<NUM_WAY; way++) {
    			//if (!block[set][way].valid)cout << block[set][way].cpu << " " ;
        		if (block[set][way].valid == false && block[set][way].cpu == cpu) {
	
		            DP ( if (warmup_complete[cpu]) {
		            std::cout << "[" << NAME << "] " << __func__ << " instr_id: " << instr_id << " invalid set: " << set << " way: " << way;
		            std::cout << hex << " address: " << (full_addr>>LOG2_BLOCK_SIZE) << " victim address: " << block[set][way].address << " data: " << block[set][way].data;
		            std::cout << dec << " lru: " << block[set][way].lru << endl; });
    	...
    	//cout << "cpu(match): ";
    	while (way == NUM_WAY && count < NUM_WAY) {
    		++count;
    		for (way=0; way<NUM_WAY; way++) {
            		//if (block[set][way].lru == NUM_WAY-count)cout << block[set][way].cpu << " " ;
            		if ((block[set][way].lru == NUM_WAY-count) && block[set][way].cpu == cpu) {
