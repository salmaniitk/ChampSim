...
uint64_t l2pf_access = 0;
    ...
    //////////////////////////////////////////////Invalidate Fill and WB /////////////////////////////////////////////// by salman

bool inc_code = true, print_Victim_Info = false, print_Invalidate_Info = false;


void CACHE::handle_inc_Fill()
{
                	uint32_t inc_set;
                	int inc_way;
			//cout << "flag value " <<  inc_flag_L1D_Fill << endl;
			if (cache_type == IS_L1I && inc_flag_L1I_Fill){
                 		inc_set = get_set(inc_address_Fill);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_Fill;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
                    			if(print_Invalidate_Info)cout << "L1I after Fill: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_Fill << dec<< endl;
                 		}
                 	}
                 	else if (cache_type == IS_L1D && inc_flag_L1D_Fill){
                 		inc_set = get_set(inc_address_Fill);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_Fill;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
                    			if(print_Invalidate_Info)cout << "L1D after Fill: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_Fill << dec<< endl;
                 		}
                 	}
                    	else if (cache_type == IS_L2C && inc_flag_L2C_Fill){
                    		inc_set = get_set(inc_address_Fill);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_Fill;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
                    			if(print_Invalidate_Info)cout << "L2C after Fill: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_Fill << dec<< endl;
                    		}
                    	}
}


void CACHE::handle_inc_WB()
{
                	uint32_t inc_set;
                	int inc_way;
			//cout << "set: way after: " <<  set << " : " << way << endl;
			if (cache_type == IS_L1I && inc_flag_L1I_WB){
                 		inc_set = get_set(inc_address_WB);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_WB;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
                    			if(print_Invalidate_Info)cout << "L1I after WB: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_WB << dec<< endl;
                 		}
                 	}
                 	else if (cache_type == IS_L1D && inc_flag_L1D_WB){
                 		inc_set = get_set(inc_address_WB);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_WB;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
		            		if(print_Invalidate_Info)cout << "L1D after WB: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_WB << dec<< endl;
                 		}
                 	}
                    	else if (cache_type == IS_L2C && inc_flag_L2C_WB){
                    		inc_set = get_set(inc_address_WB);
                    		PACKET inc_packet;
                    		inc_packet.address = inc_address_WB;
                    		inc_way = check_hit(&inc_packet);
                    		if(inc_way>=0){
                    			invalidate_entry(block[inc_set][inc_way].address);
                    			if(print_Invalidate_Info)cout << "L2C after WB: set : way : Inclusive Victim Address :: " << inc_set << " : " << inc_way << " : " << hex << inc_address_WB << dec<< endl;
                    		}
                    	}
}
...
{
/*
    ///////////////////////////////Check cache type ///////////////////// by salman
	switch(cache_type){
    		case IS_ITLB: cout << "Fill cache type: ITLB" <<  endl; break;
    		case IS_DTLB: cout << "Fill cache type: DTLB" <<  endl; break;
    		case IS_STLB: cout << "Fill cache type: STLB" <<  endl; break;
    		case IS_L1I:  cout << "Fill cache type: L1I"  <<  endl; break;
    		case IS_L1D:  cout << "Fill cache type: L1D"  <<  endl; break;
    		case IS_L2C:  cout << "Fill cache type: L2C"  <<  endl; break;
    		case IS_LLC:  cout << "Fill cache type: LLC"  <<  endl; break;
    		default: cout << "Fill UNKNOWN"  <<  endl; break;
    	}
    //////////////////////////////////////////////////////////////////////
*/
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            ...
            ////////////////////////Set Inclusive Flags////////////// by salman
            
            inc_flag_L1I_Fill = 1;
            inc_flag_L1D_Fill = 1;
            inc_flag_L2C_Fill = 1;
            inc_address_Fill = block[set][way].address;
            if(print_Victim_Info)cout <<  "LLC before Fill: set : way : Victim Address :: " <<  set << " : " << way << " : " << hex << inc_address_Fill  << dec << endl;
...
void CACHE::handle_writeback()
{
    ...
    case IS_ITLB: cout << "WB cache type: ITLB" <<  endl; break;
    case IS_DTLB: cout << "WB cache type: DTLB" <<  endl; break;
    case IS_STLB: cout << "WB cache type: STLB" <<  endl; break;
    case IS_L1I:  cout << "WB cache type: L1I"  <<  endl; break;
    case IS_L1D:  cout << "WB cache type: L1D"  <<  endl; break;
    case IS_L2C:  cout << "WB cache type: L2C"  <<  endl; break;
    case IS_LLC:  cout << "WB cache type: LLC"  <<  endl; break;
    default: cout << "WB UNKNOWN"  <<  endl; break;
    }
    ////////////////////////////////////////////////////////////////////////////
    */
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (cache_type == IS_LLC) {
          		  ...
          		  inc_flag_L1D_WB = 1;
          		  inc_flag_L2C_WB = 1;
          		  inc_address_WB = block[set][way].address;
          		  if(print_Victim_Info)cout << "LLC before Fill: set : way : Victim Address :: " <<  set << " : " << way << " : " << hex << inc_address_Fill  << dec << endl;
...
void CACHE::handle_read()
{
    ...
    case IS_ITLB: cout << "Read cache type: ITLB" <<  endl; break;
    case IS_DTLB: cout << "Read cache type: DTLB" <<  endl; break;
    case IS_STLB: cout << "Read cache type: STLB" <<  endl; break;
    case IS_L1I:  cout << "Read cache type: L1I"  <<  endl; break;
    case IS_L1D:  cout << "Read cache type: L1D"  <<  endl; break;
    case IS_L2C:  cout << "Read cache type: L2C"  <<  endl; break;
    case IS_LLC:  cout << "Read cache type: LLC"  <<  endl; break;
    default: cout << "Read UNKNOWN"  <<  endl; break;
    }
...
void CACHE::operate()
{
	...
	//cout << "CPU: " << cpu  << endl;
	///////////////////////////////Inclusive Code//////////////////////by salman
	if (cache_type == IS_LLC){
		//cout <<"Reset Inc flags" << endl;
        	inc_flag_L1I_Fill=0,inc_flag_L1D_Fill=0,inc_flag_L2C_Fill=0;
        	inc_flag_L1I_WB=0,inc_flag_L1D_WB=0,inc_flag_L2C_WB=0;
	}
